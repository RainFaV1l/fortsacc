...
const findProductById = function (productId) {
  return products.find(product => product.id === parseInt(productId))
}

onMounted(function () {
  const foundProduct = findProductById(route.params.id)
  foundProduct ? Object.assign(product, foundProduct) : ''
})
            ...
            <div class="flex flex-col gap-[113px] w-[60%]">
              <div class="flex flex-col gap-8">
                <div class="flex flex-col gap-6">
                  <h1 class="uppercase text-[42px] font-extrabold leading-tight font-['Roboto']">Ikonik Skin + <span
                      class="text-accent-green">Random Skins</span></h1>
                  <p class="w-[557px] text-accent-light-blue manrope-semibold">Best fortnite account store. Mystical
                    boxes, rare accounts, reasonable prices. All this can be bought in our store. You can also
                    participate in the distribution of skins</p>
                </div>
                <div class="flex flex-col mr-[100px]">
                  ...
                  <div class="p-5 bg-[#1C2147] border border-t-0 border-[#363C6F] rounded-[10px] rounded-t-none flex items-center justify-between gap-[43px]]">
                    <h3 class="text-accent-green text-[24px] manrope-extrabold">4.5 ETH</h3>
                    <div class="flex items-center gap-[15px]">
                      ...
                      <RouterLink :to="{ name: 'payment', params: { id: product.id } }"><button class="button-fill gradient-green text-[#151938] py-[15px] px-[55px] hover:text-white manrope-extrabold">Add to cart</button></RouterLink>
                    </div>
                  </div>
            ...
            <div class="relative flex justify-end items-center flex-col gap-[22px] w-[40%]]">
              <div class="absolute h-[755px] w-[580px] top-1/2 -translate-y-1/2 -right-[70px]">
                <div class="h-full w-full flex justify-center">
                  <img :src="product.banner" class="h-full w-full object-cover" alt="Баннер">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-[40px]">
            ...
            <div class="flex items-center justify-between gap-[30px] w-full">
              <swiper
                ...
                <swiper-slide v-for="(item, index) in popularProducts" :key="index"><ProductItem :item="{ ...item, mini: true }"/></swiper-slide>
              </swiper>
            </div>
          </div>
        </div>
